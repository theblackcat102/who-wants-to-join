...
class StackedGCNMeetup(torch.nn.Module):
    ...
    def __init__(
            ...
            input_channels=16, output_channels=1, layers=[32, 32],
            dropout=0.1):
        """
        :param args: Arguments object.
        :input_channels: Number of features.
        ...
        self.user_proj = nn.Linear(user_dim, input_channels)

        self.category_embeddings = nn.Embedding(category_size, category_dim)
        self.category_proj = nn.Linear(category_dim, input_channels)

        self.topic_embeddings = nn.Embedding(topic_size, topic_dim)
        self.topic_proj = nn.Linear(topic_dim, input_channels)

        self.group_embeddings = nn.Embedding(group_size, group_dim)
        self.group_proj = nn.Linear(group_dim, input_channels)
